...
(async function () {
    ...
    const generateExcelSheet = function (worksheet, columnTitles, arr) {
        //Makes Column Headings
        for (let i = 0; i < columnTitles.length; i++) {
            worksheet.getCell(String.fromCharCode(i + 65) + 1).value = columnTitles[i];
        }

        //Enters Data
        for (let i = 0; i < arr.length; i++) {
            for (let j = 0; j < columnTitles.length; j++) {
                worksheet.getCell(String.fromCharCode(j + 65) + (2 + i)).value = arr[i][`${columnTitles[j]}`];
            }
        }
    };

    //Make the Worksheets for Each Group
    generateExcelSheet(worksheet1, group1Keys, group1);
    generateExcelSheet(worksheet2, group2Keys, group2);
    generateExcelSheet(worksheet3, group3Keys, group3);
